...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="/difficulty-update/{{ $difficulties->id }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                    @method('put')

                    <div class="card ">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title">{{ __('Edit Difficulty') }}</h4>
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('language_code') ? ' has-danger' : '' }}">
                                        ...
                                        <option value=""></option>
                                            ...
                                            <option value="{{ $i }}" {{ $i == $difficulties->language_code ? 'selected': '' }} >{{ $locale }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('difficulty') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('difficulty') ? ' is-invalid' : '' }}"
                                            name="difficulty" id="input-difficulty" type="text" placeholder="{{ __('Difficulty') }}"
                                            value="{{ $difficulties->difficulty }}" required="true"
                                            aria-required="true" />
                                        @if ($errors->has('difficulty'))
                                        <span id="difficulty-error" class="error text-danger"
                                            for="input-difficulty">{{ $errors->first('difficulty') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
